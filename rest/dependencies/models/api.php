...
if (isset($_GET['url'])) {
  ...
  case 'user':
    if ($requestParts[1] == 'info' && indexSet(2)) {
        ...
        echo json_pretty_encode(getUserInfo($requestParts[2]));
    }
    break;
  case 'friends':
    if (indexSet(1) && indexSet(2)) {
        if ($requestParts[1] == 'get') {
            if (indexSet(3)) {
                if ($requestParts[3] == 'xml') {
                    echo xml_encode(getMutualFriends($requestParts[2]));
                    break;
                }
            }
            echo json_pretty_encode(getMutualFriends($requestParts[2]));
        }
        if ($requestParts[1] == 'requests') {
            if (indexSet(3)) {
                if ($requestParts[3] == 'xml') {
                    echo xml_encode(getFriendRequests($requestParts[2]));
                    break;
                }
            }
            echo json_pretty_encode(getFriendRequests($requestParts[2]));
        }
        if ($requestParts[1] == 'pending') {
            if (indexSet(3)) {
                if ($requestParts[3] == 'xml') {
                    echo xml_encode(getPendingFriends($requestParts[2]));
                    break;
                }
            }
            echo json_pretty_encode(getPendingFriends($requestParts[2]));
        }
    }
    break;
  case 'highscore':
    ...
    if ($requestParts[1] == 'get') {
        if (isset($_SESSION['id']) || indexSet(2)) {
            if (indexSet(2)) {
                $userID = getUserID($requestParts[2]);
            } else {
                $userID = $_SESSION['id'];
            }
